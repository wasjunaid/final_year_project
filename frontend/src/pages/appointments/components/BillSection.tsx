...
const BillSection: React.FC<BillSectionProps> = ({ appointmentId, appointmentStatus, hospitalId }) => {
  ...
  const canPatientPay = () => {
    ...
    // Patient can pay if:
    // 1. No insurance claim (direct payment)
    // 2. Insurance claim is rejected
    // 3. Insurance claim is pending (patient can choose to pay instead of waiting)
    return !bill.isClaim || 
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg space-y-3">
            ...
            {bill.claimStatus === ClaimStatus.PENDING && (
              <Alert
                ...
                message="Your insurance claim is being processed. You can choose to pay now or wait for claim approval."
                onClose={() => {}}
              />
            )}
