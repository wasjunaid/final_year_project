...
import type { CreateHospitalPayload, HospitalModel, UpdateHospitalPayload } from '../../models/hospital';
...
export const useHospitalController = () => {
  ...
  const createHospital = useCallback(
    async (payload: CreateHospitalPayload): Promise<HospitalModel | null> => {
      try {
        setLoading(true);
        setError('');
        setSuccess('');

        const newHospital = await hospitalRepository.createHospital(payload);
  ...
  // Update hospital
  const updateHospital = useCallback(async (hospitalId: number, payload: UpdateHospitalPayload): Promise<HospitalModel | null> => {
      try {
        setLoading(true);
        setError('');
        setSuccess('');

        const updatedHospital = await hospitalRepository.updateHospital(hospitalId, payload);
        setHospitals((prev) =>
