...
const AppointmentsPage: React.FC = () => {
  ...
  const { appointments, loading, error, success, fetchForPatient, fetchForDoctor, fetchForHospital, clearMessages } = useAppointmentController();

  // Clear stale errors when this page mounts
  useEffect(() => {
    clearMessages();
  }, []);
  ...
  return (
    <>
      {error && <Alert type="error" title="Error" message={error} onClose={clearMessages} className="mb-4" />}
      {success && <Alert type="success" title="Success" message="Appointment created" onClose={clearMessages} className="mb-4" />}
