...
const CreateAppointmentPage: React.FC = () => {
  ...
  const hospitalOptions = hospitals.map((h: any) => ({
    value: String(h.hospital_id ?? h.id ?? h.hospitalId),
    label: h.name ?? h.hospital_name ?? `Hospital ${h.hospital_id ?? h.id ?? h.hospitalId}`
  }));

  // apply filters: hospital (if selected)
  ...
  const doctorOptions = filteredDoctors.map((d: any) => {
    const isGarbage = (s: string) => !s || s === 'null null' || s === 'null' || s === 'undefined undefined' || s === 'undefined';

    const fullName = (d.fullName ?? '').trim();
    const name = (!isGarbage(fullName) ? fullName : null)
      || (!isGarbage(d.firstName ?? '') ? d.firstName : null)
      || (() => {
        const first = (d.doctor_first_name ?? '').replace(/^null$/i, '').trim();
        const last = (d.doctor_last_name ?? '').replace(/^null$/i, '').trim();
        return `${first} ${last}`.trim() || null;
      })();

    const email = d.email || d.doctor_email || null;

    const label = name && email
      ? `${name} (${email})`
      : name || email || `Doctor ${d.id ?? d.doctor_id}`;

    return { value: String(d.id ?? d.doctor_id), label };
  });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-2 text-xs text-gray-600 dark:text-gray-400">
                  Need a follow-up later? Open the completed appointment details and use the Create Follow-up section there.
                </p>
              </div>
        ...
        <div className="bg-white dark:bg-[#2b2b2b] rounded-xl shadow-md border border-gray-200 dark:border-[#404040] p-4 md:col-span-1">
          <div className="sticky top-4 space-y-4">
            ...
            <div>
              ...
              {hospitalId ? (
                <HospitalCard
                  hospital={(hospitals || []).find((h: any) => String(h.hospital_id ?? h.id ?? h.hospitalId) === String(hospitalId))}
                  selected
