...
export const EhrDocumentDetailsView: React.FC<EhrDocumentDetailsViewProps> = ({
  ...
  const resolvedDocumentId = (document.document_id || document.documentId || '').toString();
  ...
  useEffect(() => {
    const loadDocument = async () => {
      if (!resolvedDocumentId) {
        setDocumentError('Document ID is missing');
        return;
      }
      ...
      try {
        const blob = await DocumentService.downloadDocumentBlob(resolvedDocumentId);
        const previewBlob = document.mime_type ? new Blob([blob], { type: document.mime_type }) : blob;
  ...
  }, [resolvedDocumentId, document.mime_type]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const blobUrl = await DocumentService.downloadDocument(resolvedDocumentId);
