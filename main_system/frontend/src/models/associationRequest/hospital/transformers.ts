...
export const toHospitalAssociationRequestModel = (dto: HospitalAssociationRequestDto): HospitalAssociationRequestModel => ({
  ...
  person_name: dto.person_name || ((dto.person_first_name || dto.person_last_name) ? `${(dto.person_first_name || '').trim()} ${(dto.person_last_name || '').trim()}`.trim() : (dto.person_full_name || undefined)),
  person_email: dto.person_email || undefined,
