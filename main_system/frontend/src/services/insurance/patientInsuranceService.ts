...
import type { CreatePatientInsurancePayload, UpdatePatientInsurancePayload, ToggleAutoRenewPayload, DeactivatePatientInsurancePayload } from '../../models/insurance/payload';
...
export const patientInsuranceService = {
  ...
  async toggleAutoRenewPatientInsurance(
    patientInsuranceId: number,
    payload: ToggleAutoRenewPayload
  ): Promise<ApiResponse<PatientInsuranceDto>> {
    const response = await apiClient.put<ApiResponse<PatientInsuranceDto>>(
      `/patient-insurance/auto-renew/${patientInsuranceId}`,
      payload
    );
    return response.data;
  },
  ...
  async deactivatePatientInsurance(
    patientInsuranceId: number,
    payload: DeactivatePatientInsurancePayload = {}
  ): Promise<ApiResponse<PatientInsuranceDto>> {
    const response = await apiClient.put<ApiResponse<PatientInsuranceDto>>(
      `/patient-insurance/deactivate/${patientInsuranceId}`,
      payload
    );
    return response.data;
  },
