...
export const createPatientInsuranceRepository = ({ patientInsuranceService }: { patientInsuranceService: any }) => {
  return {
    ...
    async toggleAutoRenewPatientInsurance(patientInsuranceId: number, autoRenewEnabled: boolean): Promise<PatientInsuranceModel> {
      try {
        const response = await patientInsuranceService.toggleAutoRenewPatientInsurance(patientInsuranceId, {
          auto_renewal_enabled: autoRenewEnabled,
        });
        if (!response.success) {
          throw new AppError({
            message: response.message || 'Failed to update auto-renewal',
            title: 'Auto-Renewal Update Failed'
          });
        }
        return toPatientInsuranceModel(response.data);
      } catch (error: any) {
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to update auto-renewal';
        throw new AppError({ message: errorMessage, title: 'Auto-Renewal Update Failed' });
      }
    },

    async deactivatePatientInsurance(patientInsuranceId: number, reason?: string): Promise<PatientInsuranceModel> {
      try {
        const response = await patientInsuranceService.deactivatePatientInsurance(patientInsuranceId, {
          deactivation_reason: reason,
        });
        if (!response.success) {
          throw new AppError({
            message: response.message || 'Failed to deactivate insurance',
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to deactivate insurance';
        throw new AppError({ message: errorMessage, title: 'Deactivation Failed' });
      }
    },
  };
};
