...
const {
  validAppointmentRequestStatuses,
} = require("../../database/appointment/appointmentRequestTableQuery");
...
class AppointmentRequestService {
  static async getAppointmentRequestsForPatient(patient_id) {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        throw new AppError("No appointment requests found", statusCodes.OK);
      }

      return result.rows;
    } catch (error) {
      console.error(
        `Error fetching appointment requests: ${error.message} ${error.status}`
      );
      throw error;
    }
  }
  ...
  static async getAppointmentRequestsForHospital(person_id) {
    ...
    try {
      const checkStaff = await HospitalStaffService.checkHospitalStaffExists(
        person_id
      );
      if (!checkStaff) {
        throw new AppError(
          "You do not have permission to view appointment requests for this hospital",
          statusCodes.FORBIDDEN
        );
      }
      if (checkStaff.role === "hospital lab technician") {
        throw new AppError(
          "Lab technicians cannot view appointment requests for this hospital",
          statusCodes.FORBIDDEN
        );
      }
  ...
  static async insertAppointmentRequest(
    patient_id,
    { hospital_id, doctor_id, date, time, reason }
  ) {
    if (!patient_id) {
      throw new AppError("patient_id is required", statusCodes.BAD_REQUEST);
    }
    ...
    try {
      const checkClash = await this.checkAppointmentRequestClash(
        doctor_id,
        date,
        time
      );
      if (checkClash) {
        throw new AppError(
          "Appointment clash detected for the selected doctor at the given date and time",
          statusCodes.CONFLICT
        );
      }

      const checkDoctor =
        await DoctorService.checkDoctorExistsAgainstHospitalID(
          doctor_id,
          hospital_id
        );
      if (!checkDoctor) {
      ...
      if (result.rows.length === 0) {
        throw new AppError(
          "Failed to insert appointment request",
      ...
      console.error(
        `Error inserting appointment request: ${error.message} ${error.status}`
      );
      throw error;
    }
  }

  static async updateAppointmentRequestStatusForHospital(
    person_id,
    { appointment_request_id, doctor_id, date, time, status, cost }
  ) {
    if (!person_id) {
      throw new AppError("person_id is required", statusCodes.BAD_REQUEST);
    }
    ...
    try {
      const checkHospitalStaff =
        await HospitalStaffService.checkHospitalStaffExists(person_id);
      if (!checkHospitalStaff) {
        throw new AppError(
          "You do not have permission to update appointment request status",
          statusCodes.FORBIDDEN
        );
      }
      if (checkHospitalStaff.role === "hospital lab technician") {
        throw new AppError(
          "Lab technicians cannot update appointment request status",
          statusCodes.FORBIDDEN
        );
      }

      const checkAppointmentRequestExists =
        await this.checkAppointmentRequestExists(appointment_request_id);
      if (!checkAppointmentRequestExists) {
        throw new AppError(
          "Appointment request does not exist",
          statusCodes.NOT_FOUND
        );
      }

      if (
        checkAppointmentRequestExists.hospital_id !==
        checkHospitalStaff.hospital_id
      ) {
        throw new AppError(
          "You do not have permission to update appointment requests for this hospital",
          statusCodes.FORBIDDEN
        );
      }
      ...
      if (status === "approved") {
        ...
        await AppointmentService.insertAppointment(
          appointment_request_id,
          cost
        );
      } else if (status === "denied") {
        const appointmentExists =
          await AppointmentService.checkAppointmentExists(
            appointment_request_id
          );
      ...
      if (result.rows.length === 0) {
        throw new AppError(
          "Failed to update appointment request status",
          statusCodes.INTERNAL_SERVER_ERROR
        );
      }
      ...
      console.error(
        `Error updating appointment request status: ${error.message} ${error.status}`
      );
      throw error;
    }
  }

  static async cancelAppointmentRequest(
    patient_id,
    appointment_request_id,
    status = "cancelled"
  ) {
    ...
    try {
      const checkOwnership = await this.checkAppointmentRequestOwnership(
        patient_id,
        appointment_request_id
      );
      if (!checkOwnership) {
        throw new AppError(
          "You do not have permission to cancel this appointment request",
          statusCodes.FORBIDDEN
        );
      }
      ...
      const appointmentExists = await AppointmentService.checkAppointmentExists(
        appointment_request_id
      );
      if (appointmentExists) {
      ...
      console.error(
        `Error cancelling appointment request: ${error.message} ${error.status}`
      );
      throw error;
    }
  }

  static async rescheduleAppointmentRequest(
    patient_id,
    { appointment_request_id, date, time, reason, status = "processing" }
  ) {
    if (!patient_id) {
      throw new AppError("patient_id is required", statusCodes.BAD_REQUEST);
    }
    ...
    try {
      ...
      if (!checkOwnership) {
        throw new AppError(
          "You do not have permission to reschedule this appointment request",
          statusCodes.FORBIDDEN
        );
      }
      ...
      console.error(
        `Error resheduling appointment request: ${error.message} ${error.status}`
      );
      throw error;
    }
  }
  ...
  static async checkAppointmentRequestExists(appointment_request_id) {
    ...
    try {
      ...
      console.error(
        `Error checking appointment request exists: ${error.message} ${error.status}`
      );
      throw error;
    }
  }

  static async checkAppointmentRequestOwnership(
    patient_id,
    appointment_request_id
  ) {
    if (!patient_id) {
    ...
    try {
      ...
      console.error(
        `Error checking appointment request ownership: ${error.message} ${error.status}`
      );
      throw error;
    }
  }
  ...
  static async checkAppointmentRequestClash(doctor_id, date, time) {
    ...
    try {
      ...
      console.error(
        `Error checking appointment request clash: ${error.message} ${error.status}`
      );
      throw error;
    }
  }
