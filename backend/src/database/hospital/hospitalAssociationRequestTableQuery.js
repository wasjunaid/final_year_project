const validHospitalAssociationRequestRoles = ["doctor", "medical coder"];
...
const hospitalAssociationRequestTableQuery = `
    CREATE TABLE IF NOT EXISTS hospital_association_request (
        ...
        role VARCHAR(50) CHECK (role IN (${validHospitalAssociationRequestRoles
          .map((role) => `'${role}'`)
          .join(", ")})) NOT NULL,
...
module.exports = {
  validHospitalAssociationRequestRoles,
  hospitalAssociationRequestTableQuery,
};
